...
/**
 * Function to fetch student from an external API
...
const fetchStudentInfo = async (studentId) => {
  try {
    const response = await axios.get(
      `https://pokeapi.co/api/v2/pokemon/${studentId}`
    );
...
/**
 * Register tools (Functions) for the OpenAI API
 */

const tools = {
  fetch_student_info: async (studentId) => {
    try {
      const student = await fetchStudentInfo(studentId);
      return `Student Name: ${student.name}, Age: ${student.age}`;
    } catch (error) {
      return `Error fetching student info: ${error.message}`;
    }
  },
};
...
const processUserMessage = async (ctx, { flowDynamic, state, provider }) => {
  ...
  const response = await toAsk(ASSISTANT_ID, ctx.body, state, tools);
